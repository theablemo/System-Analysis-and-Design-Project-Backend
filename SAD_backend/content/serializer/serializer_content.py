...
class ContentSerializer(ModelSerializer):
    class Meta:
        model = Content
        fields = ['file', 'father_content', 'library', 'info', 'name']
        ...
        if 'father_content' in data:
            ...
            if 'library' in data and data['library'] != data['father_content'].library:
                raise serializers.ValidationError(
                    ...
                    code=status.HTTP_400_BAD_REQUEST)
        if 'library' in data:
            if self.instance: # is partial update
                content_type = self.instance.type
            else:
    ...
    def create(self, validated_data):
        ...
        content = Content.objects.create(
            ...
            name=validated_data['file']
        )
        return content

    def update(self, instance, validated_data):
        if 'library' in validated_data:
            instance.library = validated_data['library']
